...
- Events without stack frames omit the optional stack trace instead of sending an invalid empty array. Privacy filtering still removes messages, request bodies, identity fields, breadcrumbs and local variables. Both Sentry projects prevent IP retention for new events; Sentry can otherwise add transport IP information even after SDK-side scrubbing.
...
- Hosted negative probes passed for engine authentication, dedicated maintenance authentication and disabled provider, preview and billing routes. They exposed an app API redirect-to-login defect. After deploying its fix, the protected API returned 401 JSON with no redirect and no caching. Ryan's company access also survived a reload of that deployment.
- Sentry's existing Next.js project and a separate `inffyn-engine-alpha` project received the exact frontend and engine diagnostic event IDs from commit `692e124c2cdee6955e7eb5294ec21a409f578c1c`. Both displayed the fixed safe message with no private-evidence canary. The frontend event exposed an empty-stack-trace processing warning, corrected by the accompanying regression tests. Recheck the subsequent diagnostic after deploying that correction.
- High-priority email alerts are configured. Inbox receipt remains separate from Sentry event receipt and must be recorded only when observed. Neither an alert rule nor a successful SDK flush proves email delivery.
