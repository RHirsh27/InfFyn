...
The service-only `inffyn_private.alpha_configuration` singleton and `inffyn_private.alpha_users` table must be configured through the authorized operator SQL route after recovery and the migration/catalog checks. Insert only the pre-provisioned, verified Ryan and Stephen Auth UUIDs, match both services' `INFFYN_ALPHA_USER_IDS`, then enable the singleton in the same reviewed transaction. Check verified email/non-anonymous identity before activation. Do not grant browser access to either private table or expose allowlist values in model receipts. An absent singleton or enabled mode with an empty allowlist denies admission; an explicit disabled singleton preserves standard behavior.
